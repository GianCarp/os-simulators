...
BIN="${SCHEDSIM:-$REPO_ROOT/build/schedsim}"
UNIT_DIR="$REPO_ROOT/build/tests/schedsim"
passed=0
failed=0
...
if [ "$missing" -ne 0 ]; then
    echo "Run 'make test' to build schedsim and its unit tests."
    exit 1
fi
...
# once over the unmatched pattern. schedsim has no unit tests yet, so zero
# binaries here is expected rather than a fault.
shopt -s nullglob
